...
class Vector3:
    ...
    def project(self, other):
        """Return one vector projected on the other"""
        if isinstance(other, Vector3):
            n = other.normalized()
            return self.dot(n)*n
            ...
            B = self
            V = Vector3(0, 0, 0)
            w = B.x * A.m + B.y * A.n + B.z * A.o + A.p
            V.x = (A.a * B.x + A.b * B.y + A.c * B.z + A.d) / w
            V.y = (A.e * B.x + A.f * B.y + A.g * B.z + A.h) / w
            V.z = (A.i * B.x + A.j * B.y + A.k * B.z + A.l) / w
            return V
